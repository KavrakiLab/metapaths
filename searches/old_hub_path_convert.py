...
def update_hub_edges_dict(cmpds, cursor):
	global hub_edges_dict

	hub_edges_dict[cmpds] = []

	cursor.execute("SELECT paths FROM " + prev_hub + "_" + cmpd + ";")

	query_result = cursor.fetchall()
	if(len(query_result) > 0):

		for entry in query_result:
			#print "Entry: " + str(entry[0])

			raw_hub_paths = entry[0].split("},")
			#print "Number of Entries: " + str(len(raw_hub_paths))
			first_hub_path_cmpds = re.findall("C[0-9]{5}",raw_hub_paths[0])
				
			#print "First hub path: " + str(first_hub_path_cmpds)
			#print "Cmpds: " + str(cmpds)

			# If there are more than one path
			if len(raw_hub_paths) > 1:
				# If the first path is the same as the hub path, add the alternate edges and stop this part
				if ",".join(cmpds) == ",".join(first_hub_path_cmpds):
					#print "Adding " + str(len(raw_hub_paths)) + " subpaths"
					for raw_hub_path in raw_hub_paths:
						# Get length and atp
						atp = get_net_ATP_usage(line, cursor)
						length = len(re.findall("RP[0-9]{5}", raw_hub_path))
						hub_edges_atp_dict[cmpds].append(atp)
						hub_edges_len_dict[cmpds].append(length)
					break

	return


def get_all_hub_paths(input_cmpds_set, hub_cursor, cursor):
	global hub_edges_dict
	atp_set = []
	len_set = []

	for cmpds in input_cmpds_set:
		if cmpds not in hub_edges_dict:
			update_hub_edges_dict(cmpds, hub_cursor)

		atp_set.append(hub_edges_atp_dict[cmpds])
		len_set.append(hub_edges_len_dict[cmpds])

	final_atp_set = list(itertools.product(*atp_set))
	final_len_set = list(itertools.product(*len_set))

	return final_atp_set, final_len_set
...
def convert_lpat(filename, input_hub_list, target_cmpd):
	...
	hub_table = filename.replace(".txt", "").split("|")[-1]
	...
	hub_db = MySQLdb.connect(host="localhost",
								user="MetaDBUser",
								passwd="meta",
								db=hub_table)
